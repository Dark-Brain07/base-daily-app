...
export function WalletProvider({ children }) {
  ...
  // Initialize Coinbase Wallet SDK (v4 syntax)
  ...
  const connectCoinbase = useCallback(async () => {
    setConnecting(true); setError(null)
    try {
      // In v4, makeWeb3Provider doesn't need arguments here if appChainIds was set
      const cbProvider = coinbaseWallet.makeWeb3Provider()
      await setupWallet(cbProvider)
    } catch (e) {
      console.error('Coinbase connection error:', e)
